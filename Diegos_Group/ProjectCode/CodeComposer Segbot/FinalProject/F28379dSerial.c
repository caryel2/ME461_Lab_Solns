...
float gx = 0;
float gy = 0;
extern  int ball_found;
extern int goal_found;
extern int wall;
...
__interrupt void RXDINT_recv_ready(void)
{
    ...
    if (RXDdata & 0xC000) {
        ...
        if (com_state == 0) { //wait for data from pi
            ...
            received_count += 1;
            if (received_count == 32){  // number here is 4 * the number of variables to receive
                com_state = 0;
                received_count = 0;
                cx = received_pi[0].value;
                cy = received_pi[1].value;
                ball_found = received_pi[2].value;
                wall = received_pi[3].value;
                gx = received_pi[4].value;
                gy = received_pi[5].value;
                goal_found = received_pi[6].value;
                distance2marker = received_pi[7].value;
            }
        } else if (com_state == 2) {
            if (RXDdata == '!') {
                int i = 0;
