...
void main(void)
{
    ...
    // 200MHz CPU Freq
    //                                                  LAST PARAMETER IS PERIOD IN uSECONDS! (40,000 = .040 seconds)
...
//            serial_printf(&SerialA,"Num Timer2:%ld Num SerialRX: %ld\r\n",numTimer2calls ,numRXA);
            serial_printf(&SerialA,"Curr Read Switches:%d \r\n",currReadSwitches);
            UARTPrint = 0;
        }
    }
}
...
__interrupt void cpu_timer2_isr(void)
{
    ...
    // LAB2: Call
    currReadSwitches = ReadSwitches();
    SetLEDRowsOnOff(currReadSwitches);
    ...
    // LAB2: Set UARTPrint = 1 every time cpu_timer2_isr is triggered
    UARTPrint = 1;
