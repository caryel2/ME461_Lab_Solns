...
__interrupt void RXDINT_recv_ready(void)
{
    ...
    if (RXDdata & 0xC000) {
        ...
        numRXD ++;
        if (SerialD_state == 0) {
            dac1 = ((int16_t)RXDdata)<<8;
            SerialD_state = 1;
        } else if (SerialD_state == 1) {
            dac1 = dac1 | ((int16_t)RXDdata);
            SerialD_state = 2;
        } else if (SerialD_state == 2) {
            dac2 = ((int16_t)RXDdata)<<8;
            SerialD_state = 3;
        } else if (SerialD_state == 3) {
            dac2 = dac2 | ((int16_t)RXDdata);
            SerialD_state = 4;
        } else if (SerialD_state == 4) {
            pwm1 = ((int16_t)RXDdata)<<8;
            SerialD_state = 5;
        } else if (SerialD_state == 5) {
            pwm1 = pwm1 | ((int16_t)RXDdata);
            SerialD_state = 6;
        } else if (SerialD_state == 6) {
            dac2 = ((int16_t)RXDdata)<<8;
            SerialD_state = 7;
        } else if (SerialD_state == 7) {
            pwm2 = pwm2 | ((int16_t)RXDdata);

            DacaRegs.DACVALS.bit.DACVALS = dac1;
            DacbRegs.DACVALS.bit.DACVALS = dac2;
            EPwm3Regs.CMPA.bit.CMPA = pwm1;
            EPwm3Regs.CMPB.bit.CMPB = pwm2;
