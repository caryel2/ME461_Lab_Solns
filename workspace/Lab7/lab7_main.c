...
//            serial_printf(&SerialA,"Left Angle: %.3f, Right Angle: %.3f \r\n",leftWheelAngle,rightWheelAngle);
            serial_printf(&SerialA,"Left Velocity: %.3f, Right Velocity: %.3f, Left Dist: %.3f, Right Dist: %.3f \r\n",vLeft, vRight, posLeft,posRight);
            UARTPrint = 0;
        }
    }
}
...
__interrupt void cpu_timer1_isr(void)
{
    ...
    leftWheelAngle = readEncLeft(); // E1
    rightWheelAngle = readEncRight(); // E1

    // Calculate distant position from angle
    posLeft = leftWheelAngle * rWheel; // E1
    posRight = rightWheelAngle * rWheel; // E1

    vLeft = (posLeft - posLeft_1) / 0.004; // E2
    vRight = (posRight - posRight_1) / 0.004; // E2
	
    // errLeft = Vref - vLeft; // E3
    // errRight = Vref - vRight; // E3
    errTurn = turn + (vLeft - vRight); // E4
    errLeft = Vref - vLeft - Kp_turn*errTurn; // E4
    errRight = Vref - vRight + Kp_turn*errTurn; // E4

    // Center differencing integral for motor angle = angular velocity(0.004 is time step in seconds)
    omegaLeft = (leftWheelAngle - leftWheelAngle_1) / 0.004; // E5
    omegaRight = (rightWheelAngle - rightWheelAngle_1) / 0.004; // E5

    // E4: Integral Calculation & Wind-up Prevention
    if (uLeft >= 10 || uLeft <= -10) { // If control effort at or above max value, implies integral windup
        ILeft = ILeft_1; // Sets new integral equal to previous integral, not allowing it any higher or lower
    } else {
        ILeft = ILeft_1 + ( 0.004 * ((errLeft + errLeft_1) / 2) ); // Calculates integral normally using trapezoidal rule
    }
    if (uRight >= 10 || uRight <= -10) {
        IRight = IRight_1;
    } else {
        IRight = IRight_1 + ( 0.004 * ((errRight + errRight_1) / 2) );
    }

    uLeft = Kp*errLeft + Ki*ILeft;
    uRight = Kp*errRight + Ki*IRight;

    pose = rWheel * (rightWheelAngle - leftWheelAngle) / width; // Calculates robot angle using difference in wheel angles
    thetaAvg = 0.5 * (rightWheelAngle + leftWheelAngle); // Calculates avg wheel angle between both wheels
    omegaAvg = 0.5 * (omegaLeft + omegaRight); // Calculates avg wheel angular velocity between both wheels
    x_vel = rWheel * omegaAvg * cos(pose); // Calculates x_velocity using angle and linear speed
    y_vel = rWheel * omegaAvg * sin(pose); // Calculates y_velocity using angle and linear speed
    x_coor = x_coor_1 + (0.004 * ((x_vel + x_vel_1) / 2)); // Calculates x and y coordinates using integral of x and y-velocity
    y_coor = y_coor_1 + (0.004 * ((y_vel + y_vel_1) / 2));
    ...
    // Exit commands
    posLeft_1 = posLeft; // E2
    posRight_1 = posRight; // E2
    errLeft_1 = errLeft;
    errRight_1 = errRight;
    ILeft_1 = ILeft;
    IRight_1 = IRight;
