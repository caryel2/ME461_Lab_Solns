...
void spibRxFifoIsr(void)
{
    ...
    if(read==1){
        ...
        if(calc_offset==1){
            ...
            pitch_tilt_array[SpibNumCalls] = pitch_tilt;
            pitch_gyro_array[SpibNumCalls] = pitch_tiltrate;

            // Store Kalman Estimates for Roll
            roll_tilt_array[SpibNumCalls] = roll_tilt;
            roll_gyro_array[SpibNumCalls] = roll_tiltrate;

            // Store Kalman Estimates for Roll
            yaw_tilt_array[SpibNumCalls] = yaw_tilt;
            yaw_gyro_array[SpibNumCalls] = yaw_tiltrate;

            /*
            // Decoupled Kalman Filter needs to be debugged, since we changed the way accelerometer readings are calibrated
...
float Ky[6] = {0.05, -0.0090, 0.04, -0.0090, 0.0, 0.0}; //Working gains for Madgwick algorithm
// {0.05, -0.0198, 0.06, 0.008, 0.0, 0.0}; With KF
//
...
void balanceControl(void)
{
    ...
    cnt_u[0] = -Ky[2]*(roll_tilt_value-roll_set_point)-Ky[3]*roll_gyro_value; //
    ...
    setEPWM7A(prop_u); setEPWM7B(prop_u);
    setEPWM8A(prop_u); setEPWM8B(prop_u); */


    // Working for Madgwick algorithm
    setEPWM8A(prop_cnt[3]); //
    setEPWM8B(prop_cnt[1]); //0.0); //
    setEPWM7A(prop_cnt[0]); //0.0); //
    setEPWM7B(prop_cnt[2]); //0.0); //
...
#else
    serial_printf(&SerialA,"I:  %.4f %.4f %.4f %.4f %.4f %.4f %.1f  M:  %.2f %.2f %.2f  SA: %.2f %.2f   T: %.2f %.2f %.2f %.2f   G: %.4f %.4f %.4f %.4f   C: %.4f %.4f %.4f %.4f \n\r",
#endif
                  accelx,
                  accely,
                  accelz,
